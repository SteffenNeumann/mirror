	...
	const GLOW_BLOCKED_THEMES = new Set([
		...
		const UI_STRINGS = {
			de: {
				...
				"calendar.sidebar.booking": "Gemeinsame Planung",
				"calendar.sidebar.room": "Raum",
				"calendar.mode.personal": "Kalender",
				"calendar.mode.planning": "Planen",
			...
			en: {
				...
				"calendar.sidebar.booking": "Shared scheduling",
				"calendar.sidebar.room": "Room",
				"calendar.mode.personal": "Calendar",
				"calendar.mode.planning": "Plan",
...
<head>
  ...
  <style>
    ...
    table{border-collapse:collapse;width:100%;}
		...
		ul:not(.task-list):not(.toc-list) li,ol:not(.task-list) li{margin:0.65em 0;padding:0;}
		ul.task-list,ol.task-list{list-style:none;padding-left:0;margin:0.5em 0;}
		ul.task-list li,ol.task-list li,li.task-list-item{display:flex;gap:.55rem;align-items:flex-start;flex-wrap:nowrap;white-space:nowrap;position:relative;padding-bottom:1rem;min-width:0;transition:transform 1.8s ease;will-change:transform;color:var(--accent-text);font-size:1.1rem;font-weight:400;}
		...
		ul.task-list li.task-list-item.checked,ol.task-list li.task-list-item.checked,li.task-list-item.checked{opacity:1;color:inherit;padding-bottom:3.25rem;}
		...
		ul.task-list li > .task-closed-at,ol.task-list li > .task-closed-at,li.task-list-item > .task-closed-at{position:absolute;left:2.2rem;top:1.5rem;white-space:nowrap;}
...
<body>
	...
	function mergeServerTaskClosedTimestamps(notes) {
		if (!Array.isArray(notes)) return;
		loadTaskClosedTsCache();
		let changed = false;
		for (const note of notes) {
			if (!note || !note.taskClosedTs || typeof note.taskClosedTs !== "object") continue;
			for (const [k, v] of Object.entries(note.taskClosedTs)) {
				const num = typeof v === "number" ? v : Number(v);
				if (!Number.isFinite(num)) continue;
				const existing = taskClosedTsCache[k];
				if (!existing || num > existing) {
					taskClosedTsCache[k] = num;
					changed = true;
				}
			}
		}
		if (changed) saveTaskClosedTsCache();
	}

	function getTaskClosedTimestampsForNote(noteId) {
		const scopeKey = buildTaskScopeKey(noteId);
		if (!scopeKey) return null;
		loadTaskClosedTsCache();
		const result = {};
		let found = false;
		for (const [k, v] of Object.entries(taskClosedTsCache)) {
			if (k.startsWith(scopeKey + "|")) {
				result[k] = v;
				found = true;
			}
		}
...
`;
	...
	async function savePersonalSpaceNoteSnapshot(noteId, text, tagsPayload) {
		...
		if (!psState || !psState.authed) return false;
		try {
			const noteTaskClosedTs = getTaskClosedTimestampsForNote(targetId);
			const putBody = {
				text: rawText,
				tags: Array.isArray(tagsPayload) ? tagsPayload : [],
			};
			if (noteTaskClosedTs) putBody.taskClosedTs = noteTaskClosedTs;
