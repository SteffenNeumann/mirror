...
- **2026-09-22** Editor-Kante: Text blitzte beim Scrollen auf dem halbtransparenten `#mirror`-Rahmen auf (Overlays `inset:0` malen in den Rahmen) → `clip-path: inset(1.5px round 11px)` auf Overlays + `#psMetaOverlay`; Tags-Verlauf `right:1px`. Glasmorph für die Tags-Leiste gemessen und verworfen. **MERKE:** Ich hatte das Symptom zweimal als „alten Zeichenfehler“ abgetan — mit Pixel-Scan über echte Mausrad-Schritte (Playwright `page.mouse.wheel` + Screenshot-Clip) war es in 10 Minuten belegt. Nie ein Symptom wegerklären, das der User sieht und ich nicht nachgestellt habe.
- **2026-09-21** Tags-Leiste: Verlauf in Editor-Farbe gegen durchscheinenden Text (Folge meiner flachen Leiste). **MERKE:** Eine Deko hinter einer schmaleren Leiste deckt auf Mobil nicht — Leiste ist dort `left/right: 44px`; Verlauf deshalb am Container. (Die „Stale-Paint“-Deutung war falsch, siehe 2026-09-22.)
